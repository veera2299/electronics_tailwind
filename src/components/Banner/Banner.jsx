import React from 'react';

const Banner = ({data}) => {
    return (
        <div className='min-h-[550px] flex justify-center items-center py-12 '>
            <div className="container">
                <div style={{ backgroundColor: data.bgColor }} 
                className='grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-white rounded-3xl'>
                    {/* first col */}
                    <div className='p-8 ' >
                        <p data-aos="slide-right" className='text-sm'>{data.discount}</p>
                        <h1 data-aos="zoom-out" className='text-4xl lg:text-7xl uppercase font-bold'>{data.title}</h1>
                        <h1 data-aos= "fade-up" className='text-sm'>{data.date}</h1>
                    </div>
                    {/* second col */}
                    <div data-aos= "zoom-in"  className='h-full flex items-center'>
                        <img src={data.image} alt="" 
                        className='w-[250px] md:w-[340px] scale-125 drop-shadow-[-10px_10px_12px_rgba(0,0,0,.6)] mx-auto object-cover'
                        />
                    </div>
                    {/* third col */}
                    <div  className='flex flex-col justify-center p-6 sm:p-8 gap-4  '>
                        <p data-aos="zoom-out" className='text-xl font-bold'>{data.title2}</p>
                        <p data-aos="fade-up" className='text-3xl sm:text-5xl font-bold'>{data.title3}</p>
                        <p data-aos="fade-up" className='text-sm tracking-wide leading-5'>{data.title4}</p>
                        <div data-aos="fade-up" style={{color:data.bgColor}}>
                            <button className='bg-white rounded-full py-2 px-4'>
                                Shop Now
                            </button>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    )
}

export default Banner
